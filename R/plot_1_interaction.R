...
plot_interaction <- function(df, iv1, iv2, dv, continuous = c(F, T)){
  ...
  plot_interaction <-
    ggplot(msd_interaction, aes({{iv1}}, m, color = {{iv2}}, fill = {{iv2}}))+
    geom_line(data = msd_interaction, aes(y = m, group = {{iv2}}),
    ...
    geom_errorbar(data  = msd_interaction, aes(ymin = lower_sd, ymax = upper_sd),
  ...
  return(list(plot = plot_interaction , msd = msd_interaction))
}
